...
const { uploadFile } = require('../aws/awsConnection');
const { isValidReqBody, isValid, isValidSubcategory, isValidObjectId, isValidRelAt, isValidISBN } = require("../validator/validation");
...
const createBook = async function (req, res) {
  try {
    ...
    // if title is empty
    if (!isValid(title)) {
      return res.status(400).send({ status: false, message: "Please provide the title (required field)" });
    }
    ...
    // if excerpt is empty
    if (!isValid(excerpt)) {
      return res.status(400).send({ status: false, message: "Please provide the excerpt (required field)" });
    }

    // if userId is empty
    if (!isValid(userId)) {
      return res.status(400).send({ status: false, message: "Please enter userId (required field)" });
    }
    ...
    if (userId !== req.userId) {
      return res.status(401).send({ status: false, message: `Authorisation Failed: You are logged in as ${req.userId} not ${userId}` });
    }

    // if ISBN is empty
    if (!isValid(ISBN)) {
      return res.status(400).send({ status: false, message: "Please provide the ISBN (required field)" });
    }
    ...
    if (!isValid(category)) {
      return res.status(400).send({ status: false, message: "Please provide the category (required field)" });
    }
    ...
    if (!isValid(subcategory)) {
      return res.status(400).send({ status: false, message: "Please provide the subcategory (required field)" });
    }

    // if releasedAt is empty
    if (!isValid(releasedAt)) {
      return res.status(400).send({ status: false, message: "Please provide the book release date (required field)." });
    }

    // if releasedAt is invalid (format)
    if (!isValidRelAt(releasedAt)) {
      return res.status(400).send({ status: false, message: "Please follow the format YYYY-MM-DD for book release date" });
    }

    // if we have file in req
    ...
    // if "isDeleted": true
    if (isDeleted) {
      return res.status(400).send({ status: false, message: "isDeleted cannot be true during creation!" });
    }
    ...
    // response
    res.status(201).send({ status: true, message: "Sucess", data: createdBook });
  } catch (err) {
    res.status(500).send({ status: false, message: "Internal Server Error", error: err});
  }
};
...
let getBooks = async function (req, res) {
  try {
    ...
    let findBook = await bookModel.find(filter)
      .select({ title: 1, book_id: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1 });
...
let getBooksById = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(bookId)) {
      return res.status(400).send({ status: true, message: "bookId is required in params" });
    }
...
const updateBook = async function (req, res) {
  try {
    ...
    if (bodyFromReq.hasOwnProperty("releasedAt")) {
      ...
      if (!isValidRelAt(releasedAt)) {
        return res.status(400).send({ status: false, message: "releasedAt is not valid.Please use (YYYY-MM-DD) format" });
      }
    }
    ...
    const updatedBook = await bookModel.findOneAndUpdate({ _id: bookId }, { ...bodyFromReq }, { new: true });
...
const deleteBook = async function (req, res) {
  try {
    ...
    if (check && !check.isDeleted) {
      ...
      await bookModel.findOneAndUpdate({ _id: bookId }, { isDeleted: true, deletedAt: new Date() });

      return res.status(200).send({ status: true, message: "Deletion Successful" });
    }
